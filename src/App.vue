<template>
  <div class="bg-gray-950 text-white text-center min-h-screen flex flex-col p-4">
    <!--    Header-->
    ...
    <!--    Main-->
    <main class="container mx-auto p-6 flex-1 md:container md:mx-auto">
      <div
        v-if="userChoice === null"
        class="flex items-center justify-center mx-6 :xs flex-col :xs flex-col gap-2"
      >
        <!--        Buttons-->
        <button
          @click="play('rock')"
        ...
        >
          <img src="./assets/RockIcon.svg" alt="Rock" class="w-full :sm " />
        </button>
        <button
          @click="play('paper')"
          class="bg-white rounded-full shadow-lg w-64 p-12 mx-6 transition-colors duration-300 hover:bg-blue-500"
      ...
      <div v-else>
        ...
        <div class="text-3xl mb-4">
          The Computer picked
          <span class="text-pink-500"> {{ computerChoice }}</span>
        </div>
        ...
        <button
          @click="ResetRound"
          class="bg-blue-500 py-2 px-4 rounded-md text-lg"
        >
          Reset
        </button>
      </div>
